...
import com.likelion.timer.timer.domain.repository.RecordTimerRepository;
import com.likelion.timer.timer.domain.repository.TimerRepository;
...
public class TimerService {
	...
	private List<PartList> checkPartLists(Boolean isSettingByUser, List<PartReqDto> parts) {
		...
		if (isSettingByUser) { // 직접 설정인 경우
			...
			partLists = partListService.saveRandomUniqueParts(RANDOM.nextInt(TimerConstants.MAX_STRETCHING_TIMES) + 1);
		}
